...
if not st.session_state.get("pot_log"):
    st.session_state["pot_log"] = []
...
def get_global_stats():
    ...
    total_bet_amount = sum(bet["bet_size"] for bet in pot_log if "bet_size" in bet and isinstance(bet["bet_size"], (int, float)))
    average_bet_size = total_bet_amount / total_bets if total_bets > 0 else 0
    largest_bet = max((bet["bet_size"] for bet in pot_log if "bet_size" in bet and isinstance(bet["bet_size"], (int, float))), default=0)
    smallest_bet = min((bet["bet_size"] for bet in pot_log if "bet_size" in bet and isinstance(bet["bet_size"], (int, float))), default=0)
